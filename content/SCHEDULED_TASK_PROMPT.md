...
2. **vocab**：收所有名詞・動詞・形容詞・副詞（不收助詞/助動詞）。key 必須與 token 的 basic_form 完全一致。`reading` 只在 **key 含漢字時**才填平假名讀音（例：`"食べる": { ..., "reading": "たべる" }`）；**純假名 key 不需要 reading 欄位**。生成後自查：掃描所有 token，確認每個名詞/動詞/形容詞/副詞的 basic_form 都有對應 vocab key。**若 token 不在 vocab 中，App 會顯示「尚無中文翻譯」，這是錯誤。**
